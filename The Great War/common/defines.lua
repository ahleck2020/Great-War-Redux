...
country = {
	...
	NEWSPAPER_PRINTING_FREQUENCY = 0, -- days frequency when the news attempt to be printed (may not print if not enough facts collected, and retry after another X days)
	NEWSPAPER_TIMEOUT_PERIOD = 0, -- the max period for news that may be printed. The probability is decreased with time.
	NEWSPAPER_MAX_TENSION = 0, -- when tension of printing gets this high, the newspaper will attempt to be printed immediately.
...
economy = {
	...
	RGO_SUPPLY_DEMAND_FACTOR_HIRE_HI = 5,	-- how fast pops are Hired when there is a high demand
	RGO_SUPPLY_DEMAND_FACTOR_HIRE_LO = 5,	-- how fast pops are Hired when there is a medium demand
	RGO_SUPPLY_DEMAND_FACTOR_FIRE = 0.0001,		-- how fast pops are Fired when there is a low demand
...
military = {
	...
	AI_BLOCKADE_RANGE = 1000, -- max distance the AI will send out blockade fleets from their home base
...
diplomacy = {
	...
	TWS_FULFILLED_SPEED = 0.25, -- Ticking War Score grows up with this speed daily once CB is fulfilled
	TWS_NOT_FULFILLED_SPEED = 0.25, -- Ticking War Score falls down when CB is not fulfilled (or after grace period)
...
ai =
{
	...
	NAVAL_SUPPLY_PENALTY_LIMIT = 0.4, -- AI will allow to have max X% of supply penalty (when too little naval bases)
